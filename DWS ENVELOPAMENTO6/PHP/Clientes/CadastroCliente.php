...
try {
    if ($clifoto !== null) {
        try {
            ...
            // A coluna "clifoto" ainda não existe na tabela `clientes` do banco
            // enviado (dwsatualizado.sql não tem essa coluna). Em vez de quebrar
            // o cadastro inteiro por causa da foto, cadastra sem foto.
            // Para a foto funcionar de verdade, rode:
            //   ALTER TABLE clientes ADD COLUMN clifoto VARCHAR(255) DEFAULT NULL;
            if (strpos($eFoto->getMessage(), 'clifoto') === false) {
                throw $eFoto; // erro diferente, não é a coluna faltando - relança
            }
