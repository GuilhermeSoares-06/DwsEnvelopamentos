...
// Também corrigido o SQL Injection que existia aqui antes, que concatenava $_GET['id']
// direto na query.)
if(isset($_GET['id'])) {
    $id = (int)$_GET['id'];
    $stmt_busca = $pdo->prepare("SELECT * FROM clientes WHERE cliid = :id");
...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    $id = (int)$_POST['id'];
    // Aqui também não se deve escapar manualmente antes de usar parâmetros -
    // isso duplicava a escapagem e gravava barras invertidas indevidas no banco.
    ...
    $erroMsg = '';
    try {
        // Se a senha foi preenchida, atualiza com a nova senha (com hash)
        if(!empty($senha)) {
            $senha = password_hash($senha, PASSWORD_DEFAULT);
            $stmt = $pdo->prepare("UPDATE clientes SET clinome = :n, clisenha = :s, clicpf = :c, clitel = :t, cliendereco = :e WHERE cliid = :id");
            $sucesso = $stmt->execute([':n'=>$nome, ':s'=>$senha, ':c'=>$cpf, ':t'=>$telefone, ':e'=>$endereco, ':id'=>$id]);
        } else {
            // Se a senha está vazia, mantém a senha atual
            $stmt = $pdo->prepare("UPDATE clientes SET clinome = :n, clicpf = :c, clitel = :t, cliendereco = :e WHERE cliid = :id");
            $sucesso = $stmt->execute([':n'=>$nome, ':c'=>$cpf, ':t'=>$telefone, ':e'=>$endereco, ':id'=>$id]);
        }
    ...
    if($sucesso) {
        ...
        ";
        exit(); // sem isso, o formulário de edição era renderizado de novo por baixo
    } else {
        echo "<script>
            alert('Erro ao atualizar: " . addslashes($erroMsg) . "');
            window.history.back();
        </script>";
        exit(); // idem: sem isso, tentava montar o form abaixo sem garantia de $cliente
    }
}

// Se chegou até aqui sem ter carregado um cliente (ex: acessou a página sem
// ?id=... na URL), evita warnings do PHP e mostra uma mensagem clara.
if (!isset($cliente)) {
    die("Cliente não encontrado. Acesse esta página informando o ID do cliente (ex: AlterarCliente.php?id=1).");
}
?>
